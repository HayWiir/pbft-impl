defmodule Raft do
  ...
  * The second element is the updated state.
  ...
  @doc """
  Called by previous leader. Sends new leader to all non commited logs.
  """
  @spec send_new_leader(%Raft{is_leader: false}, non_neg_integer()) ::
          no_return()
  def send_new_leader(state = %Raft{is_leader: false}, curr_log_index) do
    if curr_log_index <= get_last_log_index(state) do
      some_client = get_log_entry(state, curr_log_index).requester
      send(some_client, {:redirect, state.current_leader})
      send_new_leader(state, curr_log_index + 1)
    end
  end

  @doc """
  Called by leader. Commits all log entries for which majority responses
  are received.
  """
  @spec commit_log(%Raft{is_leader: true}, %{}) :: {%Raft{}}
  def commit_log(state = %Raft{is_leader: true}, extra_state) do
    if Map.has_key?(extra_state, state.commit_index + 1) and
         extra_state[state.commit_index + 1] >= div(length(state.view), 2) + 1 do
      {{client, msg}, state} = commit_log_index(state, state.commit_index + 1)
      ...
      IO.puts(
        "#{whoami} committed log entry #{state.commit_index} with response #{inspect({client, msg})}"
      )

      commit_log(state, extra_state)
    else
      state
    end
  end
  ...
  @doc """
  Called by follower. Commits all log entries till leader's commit_index.
  """
  @spec commit_log(%Raft{is_leader: false}, non_neg_integer()) :: {%Raft{}}
  def commit_log(state = %Raft{is_leader: false}, leader_commit_index) do
    if state.commit_index < leader_commit_index do
      {{client, msg}, state} = commit_log_index(state, state.commit_index + 1)
      ...
      commit_log(state, leader_commit_index)
    else
      state
    end
  end
  ...
  @spec cancel_election_timer(%Raft{}) :: %Raft{}
  ...
  # Cancel the hearbeat timer.
  @spec cancel_heartbeat_timer(%Raft{}) :: %Raft{}
  defp cancel_heartbeat_timer(state) do
    if not Kernel.is_nil(state.heartbeat_timer) do
      Emulation.cancel_timer(state.heartbeat_timer)
    end

    save_heartbeat_timer(state, nil)
  end
  ...
  # Utility function to send uncommited log entries to all
  # processes other than the caller. Should only be used by leader.
  @spec broadcast_log_entries(%Raft{is_leader: true}) :: [boolean()]
  defp broadcast_log_entries(state) do
    me = whoami()
    ...
    |> Enum.map(fn pid ->
      send(pid, %Raft.AppendEntryRequest{
        ...
        leader_id: me,
        prev_log_index: state.next_index[pid] - 1,
        prev_log_term: get_log_entry(state, state.next_index[pid] - 1),
  ...
  def follower(state, extra_state) do
    receive do
      ...
      :timer ->
        IO.puts("#{inspect(whoami())} election timer went off")
      ...
      {sender,
       %Raft.AppendEntryRequest{
        ...
        curr_term = state.current_term
        current_leader = state.current_leader

        case {term, leader_id, prev_log_index, prev_log_term, entries,
              leader_commit_index} do
          # Hearbeat
          {^curr_term, ^current_leader, _, _, [], _} ->
            state = reset_election_timer(state)
            state = commit_log(state, leader_commit_index)

            IO.puts(
              "#{inspect(whoami())} received hearbeat from #{inspect(current_leader)}"
            )

            follower(state, extra_state)

          {^curr_term, ^current_leader, _, _, [_ | _], _} ->
            if prev_log_index > 0 and
                 not (logged?(state, prev_log_index) or
                        get_log_entry(state, prev_log_index).term !=
                          prev_log_term) do
              send(sender, %Raft.AppendEntryResponse{
                term: curr_term,
                log_index: get_last_log_index(state),
                success: false
              })
            ...
            state = %{
              state
              | commit_index:
                  min(leader_commit_index, get_last_log_index(state))
            }

            IO.puts(
              "#{inspect(whoami())} received Entries for term #{term} with leader #{leader_id} " <>
                "(#{leader_commit_index}) and current log : #{inspect(state.log)}"
            )
          ...
          {_, ^current_leader, _, _, _, _} ->
            if curr_term > term do
              send(sender, %Raft.AppendEntryResponse{
                term: curr_term,
                log_index: get_last_log_index(state),
                success: false
          ...
          # New Leader Hearbeat
          {^curr_term, _, _, _, [], _} when leader_id != current_leader ->
            state = reset_election_timer(state)
            state = %{state | voted_for: nil}

            # if previous leader
            if whoami == state.current_leader do
              send_new_leader(state, state.commit_index + 1)
              state = truncate_log_at_index(state, state.commit_index)
              IO.puts("#{inspect(whoami())} current log #{inspect(state.log)}")
              state = update_leader(state, leader_id)

              IO.puts(
                "#{inspect(whoami())} set leader to #{inspect(leader_id)}"
              )
      ...
      {sender,
       %Raft.AppendEntryResponse{
        ...
        # a follower.
        IO.puts(
          "Follower #{inspect(whoami)} received append entry response #{term} from #{inspect(sender)}," <>
      ...
      {sender,
       %Raft.RequestVote{
        ...
        if term > state.current_term do
          ...
          if (state.voted_for == nil or state.voted_for == candidate) and
               (last_log_term > get_last_log_term(state) or
                  (last_log_term == get_last_log_term(state) and
                     last_log_index >= get_last_log_index(state))) do
            response_message =
              Raft.RequestVoteResponse.new(state.current_term, true)

            send(sender, response_message)
            ...
            response_message =
              Raft.RequestVoteResponse.new(state.current_term, false)
  ...
  def become_leader(state) do
    ...
    IO.puts("#{inspect(whoami())} become leader")
    ...
    receive do
      ...
      {sender,
       %Raft.AppendEntryRequest{
        ...
        IO.puts(
          "Leader Received append entry for term #{term} with leader #{leader_id} " <>
            "(#{leader_commit_index})"
        )
        ...
        cond do
          ...
          term == state.current_term ->
            raise "Multiple leaders!!"

          term > state.current_term ->
            state = reset_heartbeat_timer(state)
            state = %{state | current_term: term}
            state = %{state | current_leader: nil, voted_for: nil}
            # send_new_leader(state, state.commit_index+1)
            IO.puts("#{inspect(whoami())} set leader to #{inspect(leader_id)}")

            # TODO: Handle actual request
            case entries do
              [] ->
                # heartbeat/inital leader annoucement
                become_follower(state)

              [_ | _] ->
                # actual entries 
                # IO.puts("#{inspect(whoami())} sending appendentry response to #{inspect(sender)}")
                message =
                  Raft.AppendEntryResponse.new(
                    state.current_term,
                    get_last_log_index(state),
      ...
      {sender,
       %Raft.AppendEntryResponse{
        ...
        if succ do
          state = %{
            state
            | next_index: %{state.next_index | sender => index + 1},
              match_index: %{state.match_index | sender => index}
          }

          extra_state = %{extra_state | index => extra_state[index] + 1}
          state = commit_log(state, extra_state)
      ...
      {sender,
       %Raft.RequestVoteResponse{
        ...
        if term <= state.current_term do
          ...
          raise "Recieved RequestVoteResponse with higher term"
        end
      ...
      {sender, :nop} ->
        ...
        IO.puts(
          "Leader #{whoami} Received nop command for term #{state.current_term} from #{sender} "
        )

        state = add_log_entries(state, [entry])
        extra_state = Map.put(extra_state, get_last_log_index(state), 0)
      ...
      {sender, {:enq, item}} ->
        ...
        IO.puts(
          "Leader #{whoami} Received enq command for term #{state.current_term} from #{sender} "
        )
      ...
      {sender, :deq} ->
        ...
        IO.puts(
          "Leader #{whoami} Received deq command for term #{state.current_term} from #{sender} "
        )
    ...
    receive do
      ...
      {sender,
       %Raft.AppendEntryRequest{
        ...
        if term >= state.current_term do
          ...
          state = reset_election_timer(state)
          state = %{state | current_term: term}
          state = %{state | current_leader: leader_id, voted_for: nil}
          IO.puts("#{inspect(whoami())} set leader to #{inspect(leader_id)}")
      ...
      {sender,
       %Raft.RequestVote{
        ...
        if term > state.current_term do
          ...
          if last_log_term > get_last_log_term(state) or
               (last_log_term == get_last_log_term(state) and
                  last_log_index >= get_last_log_index(state)) do
            response_message =
              Raft.RequestVoteResponse.new(state.current_term, true)
      ...
      {sender,
       %Raft.RequestVoteResponse{
        ...
        IO.puts(
          "Candidate received RequestVoteResponse from #{inspect(sender)} " <>
            "term = #{term}, granted = #{inspect(granted)}"
        )
